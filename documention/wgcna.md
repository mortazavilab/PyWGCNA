...
|---|---|---|---|
| name | str | name of the WGCNA we used to visualize data (default: 'WGCNA') | None |
| geneExpression | class GeneExp | gene expression class
    geneExpPath: str
    geneExp: dataframe
    sep: str | None |
| datExpr | anndata | clean gene expression matrix with genes and sample information | None |
| TPMcutoff | int | cut off for removing genes with out any specific expression | None |
| power | int | power used to have scale free network (default: 6) | 6 |
| networkType | str | Type of network we can create (default: "signed hybrid")
possible Type: "unsigned", "signed" and "signed hybrid" | None |
| TOMType | str | Type of topological overlap matrix(TOM) (default: "signed")
possible Type: "unsigned", "signed" | None |
| minModuleSize | int | We like large modules, so we set the minimum module size relatively high (default: 50) | 50 |
| naColor | str | color we used to identify genes we don't find any cluster for them (default: "grey") | None |
| cut | float | number to remove outlier sample (default: 'inf')
By default we don't remove any sample by hierarchical clustering | we |
| MEDissThres | int | (default: 0.2) | 0.2 |
| save | bool | define wether you want to save result of important steps or not (default: False) | False |
| outputPath | str | if save is TRUE, define where you want to save your data (default: '') | None |
| Methods | None | None | None |
| ------- | None | None | None |
| main iner function to run PyWGCNA |  | checkAndScaleWeights()
    check and scale weights of gene expression
...
mergeCloseModules()
    Merges modules in gene expression networks that are too close as measured by the correlation of their eigengenes. | None |
| getGeneModule() | None | return list of module corresponding to gene | None |
| addGeneList() | None | create gene list for each module | None |
| updateDatTraits() | None | update data trait module base on samples | None |
| plotModuleEigenGene() | None | plot module eigen gene figure in given module | None |
| saveWGCNA() | None | Saves the current WGCNA in pickle format with the .p extension | None |
...
def cutreeHybrid(
    ...
    externalSplitOptions=Empty DataFrame
Columns: []
Index: [],
...
```python3
def equalizeQuantilesFun(
    data,
    summaryType=['median', 'mean']
)
```
